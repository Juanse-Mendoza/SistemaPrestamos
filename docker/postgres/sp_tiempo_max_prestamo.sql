-- =============================================================================
-- Migración: tiempo máximo de préstamo por artículo
-- =============================================================================

\connect prestamos_umb

-- 1) Columna nueva en articulos
ALTER TABLE articulos
    ADD COLUMN IF NOT EXISTS tiempo_maximo_dias INTEGER;

COMMENT ON COLUMN articulos.tiempo_maximo_dias IS
  'Número máximo de días que se puede prestar este artículo. NULL = sin límite.';

-- 2) Recrear vista v_stock_disponible para incluir el campo
...
-- 3) Reemplazar sp_registrar_articulo para aceptar tiempo_maximo_dias
DROP PROCEDURE IF EXISTS sp_registrar_articulo(VARCHAR, TEXT, INTEGER, INTEGER, VARCHAR, VARCHAR, VARCHAR);
...
BEGIN
    ...
    IF p_codigo_barras IS NOT NULL AND EXISTS (
        ...
        RETURN;
    END IF;
    IF p_tiempo_maximo_dias IS NOT NULL AND p_tiempo_maximo_dias <= 0 THEN
        p_articulo_id := -1;
    ...
    INSERT INTO articulos (nombre, descripcion, categoria_id, stock_total, stock_disponible,
                           codigo_interno, ubicacion, codigo_barras, tiempo_maximo_dias)
    VALUES (p_nombre, p_descripcion, p_categoria_id, p_stock_total, p_stock_total,
            p_codigo_interno, p_ubicacion, p_codigo_barras, p_tiempo_maximo_dias)
...
-- 4) Trigger fn_validar_prestamo: validar que la fecha no exceda el máximo del artículo
...
DECLARE
    ...
    v_stock      INTEGER;
    v_max_dias   INTEGER;
    v_dias_pedidos NUMERIC;
BEGIN
    SELECT estado, stock_disponible, tiempo_maximo_dias
    INTO v_estado, v_stock, v_max_dias
    FROM articulos
    ...
    -- Validar tiempo máximo de préstamo del artículo
    IF v_max_dias IS NOT NULL THEN
        v_dias_pedidos := EXTRACT(EPOCH FROM (NEW.fecha_devolucion_esperada - NEW.fecha_prestamo)) / 86400.0;
        IF v_dias_pedidos > v_max_dias THEN
            RAISE EXCEPTION 'El préstamo no puede exceder % día(s). Solicitado: % día(s).',
                v_max_dias, ROUND(v_dias_pedidos::numeric, 1);
        END IF;
    END IF;
